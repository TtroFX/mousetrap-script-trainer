...
function validateSpeechMap(value, name) {
  ...
  for (const scene of SCENES) for (let i = 1; i <= scene.count; i += 1) expected.add(`${scene.id}-speech-${String(i).padStart(4, '0')}`);
...
function validateStructure(value) {
  if (!value || typeof value !== 'object' || value.schemaVersion !== 1) throw new Error('structure: schema invalid');
  if (value.counts?.speeches !== 1164 || value.counts?.sentences !== 2277 || value.counts?.chunks !== 8055) throw new Error('structure: count contract invalid');
  return value;
}
...
export class DataStore extends EventTarget {
  ...
  getStructure(lineId) { return this.structure?.lines?.[lineId] || this.structure?.rawLines?.[lineId] || null; }
