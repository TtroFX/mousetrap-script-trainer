...
const FILES = {
  ...
  patchMeta: 'data/first-quarter-noun-expansion-v2/meta.json',
  patchParts: ['data/first-quarter-noun-expansion-v2/part-01.json','data/first-quarter-noun-expansion-v2/part-02.json','data/first-quarter-noun-expansion-v2/part-03.json','data/first-quarter-noun-expansion-v2/part-04.json'],
  report: 'data/first-quarter-noun-expansion-v2-qa.json'
};

const patch = readJson(FILES.patchMeta);
patch.lineRules = FILES.patchParts.flatMap(readJson);
patch.dictionaryOnly = [];
if (patch?.schemaVersion !== 1 || patch?.patchId !== 'first-quarter-noun-expansion-v2-2026-09-01') fail('Unexpected patch identity');
const scope = patch.scope || {};
if (JSON.stringify(scope.globalSpeechRange) !== JSON.stringify([1, 291]) || scope.speechCount !== 291) fail('Unexpected patch scope');
if (scope.first !== 'act1-scene1-speech-0001' || scope.last !== 'act1-scene2-speech-0101' || scope.nextExcluded !== 'act1-scene2-speech-0102') fail('Unexpected scope boundary');
if (!Array.isArray(patch.lineRules) || patch.lineRules.length < 150) fail('Part 1 expansion rule set unexpectedly small');
...
const targetSpeeches = [...script['act1-scene1'].speeches, ...script['act1-scene2'].speeches.slice(0, 101)];
...
for (const rule of patch.lineRules) {
  ...
  if (allowed && [...allowed].some(id => !targetIds.has(id))) fail(`Rule speechIds escape Part 1: ${surface}`);
...
if (stats.newVocabularyItems < 1) fail('Part 1 expansion produced no new line vocabulary');
