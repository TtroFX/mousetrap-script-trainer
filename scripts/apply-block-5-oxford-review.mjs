...
const review = read('data/vocabulary-rebuild/block-5-oxford-review.json');
const script = read('mousetrap_script_data.json');
const contextPath = 'data/vocabulary-rebuild/block-5-line-vocabulary.json';
const thresholdPath = 'data/vocabulary-rebuild/block-5-b1plus-coverage.json';
const dictionaryPath = 'data/vocabulary-rebuild/block-5-dictionary.json';
...
for (const lex of review.includeLexemes || []) {
  ...
  for (const speechId of targetIds) {
    ...
    if (!speech) throw new Error(`Review speech outside Block 5: ${speechId} / ${lex.word}`);
...
write('data/vocabulary-rebuild/block-5-oxford-application.json', {
  schemaVersion: 1,
  blockId: 'block-5',
