...
const part3=analyseQuarter(583,873,'PART 3 / 583-873');
const part4=analyseQuarter(874,1164,'PART 4 / 874-1164');
...
const combined=combine([['3',part3],['4',part4]]);
const combinedText=[
  'THE MOUSETRAP — A2+ ADDITION CANDIDATES — PARTS 3+4 UNIQUE',
  'Scope: global speeches 583-1164',
  'Pipeline: extract Oxford A2+ occurrences -> deduplicate by headword -> remove currently implemented dictionary headwords -> deduplicate across Parts 3 and 4.',
  `Unique final Oxford A2+ candidates across Parts 3+4: ${combined.length}`,
  '',
...
fs.writeFileSync('data/a2plus-candidate-lists/part-03.txt',part3.text+'\n');
fs.writeFileSync('data/a2plus-candidate-lists/part-04.txt',part4.text+'\n');
fs.writeFileSync('data/a2plus-candidate-lists/part-03-04-unique.txt',combinedText+'\n');
const summary={
  schemaVersion:1,
  status:'PASS',
  source:OXFORD_URL,
  ...
  parts:{
    part3:{scope:[583,873],...part3.stats},
    part4:{scope:[874,1164],...part4.stats},
  },
  uniqueOxfordA2PlusAcrossParts3And4:combined.length,
};
fs.writeFileSync('data/a2plus-candidate-lists/summary-part-03-04.json',JSON.stringify(summary,null,2)+'\n');
