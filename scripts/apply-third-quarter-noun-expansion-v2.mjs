...
const FILES = {
  ...
  patchMeta: 'data/third-quarter-noun-expansion-v2/meta.json',
  patchParts: ['data/third-quarter-noun-expansion-v2/part-01.json','data/third-quarter-noun-expansion-v2/part-02.json','data/third-quarter-noun-expansion-v2/part-03.json','data/third-quarter-noun-expansion-v2/part-04.json'],
  report: 'data/third-quarter-noun-expansion-v2-qa.json'
};

const patch = readJson(FILES.patchMeta);
patch.lineRules = FILES.patchParts.flatMap(readJson);
patch.dictionaryOnly = [];
if (patch?.schemaVersion !== 1 || patch?.patchId !== 'third-quarter-noun-expansion-v2-2026-09-01') fail('Unexpected patch identity');
const scope = patch.scope || {};
if (JSON.stringify(scope.globalSpeechRange) !== JSON.stringify([583, 873]) || scope.speechCount !== 291) fail('Unexpected patch scope');
if (scope.first !== 'act2-speech-0057' || scope.last !== 'act2-speech-0347' || scope.nextExcluded !== 'act2-speech-0348') fail('Unexpected scope boundary');
if (!Array.isArray(patch.lineRules) || patch.lineRules.length < 80) fail('Part 3 expansion rule set unexpectedly small');
...
const targetSpeeches = script['act2'].speeches.slice(56, 347);
...
for (const rule of patch.lineRules) {
  ...
  if (allowed && [...allowed].some(id => !targetIds.has(id))) fail(`Rule speechIds escape Part 3: ${surface}`);
...
if (stats.newVocabularyItems < 1) fail('Part 3 expansion produced no new line vocabulary');
