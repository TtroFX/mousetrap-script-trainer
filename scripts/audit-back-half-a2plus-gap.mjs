...
for(const row of rows){
  ...
  for(const speech of target){
    ...
    if(matched.length) contexts.push({speechId:speech.id,speaker:speech.speaker,matchedForms:matched,text:speech.text});
  }
  const item={...row,mixedA1:String(row.allOxfordLevels||'').split('/').includes('A1'),contexts};
