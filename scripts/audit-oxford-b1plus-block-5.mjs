...
const context = read('data/vocabulary-rebuild/block-5-line-vocabulary.json');
const threshold = read('data/vocabulary-rebuild/block-5-b1plus-coverage.json');
const reviewPath = 'data/vocabulary-rebuild/block-5-oxford-review.json';
const reportPath = 'data/vocabulary-rebuild/block-5-oxford-audit.json';
const review = fs.existsSync(reviewPath) ? read(reviewPath) : { includeLexemes: [], excludeWords: {} };
const speeches = script.act2.speeches.slice(179, 423);
...
for (const [word, entries] of oxford.entries()) {
  ...
  if (covered) continue;
  rawMissing.push({word,oxfordEntries:entries,b1plusEntries:b1plus,observedForms:[...new Set(uses.map(x=>x.form))].sort(),occurrences:uses.slice(0,20)});
}
...
for (const lex of review.includeLexemes||[]) {
  ...
  const restricted = new Set(lex.speechIds || []);
  for (const speech of speeches) {
    if (restricted.size && !restricted.has(speech.id)) continue;
    const tokens=[...speech.text.toLowerCase().matchAll(/[a-z]+(?:['’][a-z]+)?/g)].map(m=>norm(m[0]));
    for (const form of forms) if (tokens.includes(form) && ![...selected].some(x=>x===norm(lex.lemma)||x===form)) coverageErrors.push({speechId:speech.id,lemma:lex.lemma,form});
  }
}
const report={source:OXFORD_URL,parsedOxfordWords:oxford.size,blockId:'block-5',speeches:speeches.length,selectedKeys:selected.size,rawMissingCount:rawMissing.length,reviewedExclusionCount:Object.keys(excluded).length,unresolvedCount:unresolved.length,includeLexemeCoverageErrorCount:coverageErrors.length,unresolved,includeLexemeCoverageErrors:coverageErrors,rawMissing};
