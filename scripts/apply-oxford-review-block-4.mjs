...
const review = read('data/vocabulary-rebuild/block-4-oxford-review.json');
const context = read('data/vocabulary-rebuild/block-4-line-vocabulary.json');
const threshold = read('data/vocabulary-rebuild/block-4-b1plus-coverage.json');
const dictionary = read('data/vocabulary-rebuild/block-4-dictionary.json');

if (review.blockId !== 'block-4') throw new Error(`Unexpected review block: ${review.blockId}`);
if (threshold.blockId !== 'block-4' || dictionary.blockId !== 'block-4' || context.blockId !== 'block-4') {
  throw new Error('Block 4 generated inputs are inconsistent.');
}

const speeches = script.act2.speeches.slice(0, 179);
...
for (const lex of review.includeLexemes || []) {
  ...
  for (const speechId of requestedIds) {
    ...
    if (!speech) {
      failures.push({ word:lex.word, speechId, reason:'speechId is outside Block 4.' });
      continue;
    }
  ...
  if (!matchedIds.length) failures.push({ word:lex.word, reason:'No Block 4 occurrence matched reviewed forms.' });
  if (!dictionary.entries[lemma]) {
    dictionary.entries[lemma] = {
      ...
      ...(Array.isArray(lex.tags) && lex.tags.length ? { tags:lex.tags } : {})
    };
  }
...
write('data/vocabulary-rebuild/block-4-b1plus-coverage.json', threshold);
write('data/vocabulary-rebuild/block-4-dictionary.json', dictionary);
write('data/vocabulary-rebuild/block-4-oxford-application.json', {
  schemaVersion:1,
  blockId:'block-4',
