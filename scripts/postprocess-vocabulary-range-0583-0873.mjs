...
const lookup=s=>String(s||'').trim().toLowerCase();
...
const dmap=new Map(Object.entries(dict).map(([k,d])=>[lookup(d.lemma||k),d]));
...
for(const [id,rows] of Object.entries(vocab)){
 ...
 const seen=new Set();
 for(const e of rows){total++; if(e.playMeaning===true)playMeaning++; if('inThisPlay'in e)inThisPlay++; refs.add(lookup(e.lemma));
   if(idSet.has(id)){const k=key(e);if(seen.has(k))fail('target duplicate '+id+' '+k);seen.add(k);}
   const d=dmap.get(lookup(e.lemma));if(!d)fail('missing dictionary '+e.lemma);if(String(e.meaning||'').trim()!==String(d.meaning||'').trim())fail('meaning mismatch '+id+' '+e.lemma);
 }
}
const baseDMap=new Map(Object.entries(baseD).map(([k,d])=>[lookup(d.lemma||k),d]));
...
for(const id of ids){const before=baseV[id]||[],after=vocab[id]||[];baseTargetItems+=before.length;targetItems+=after.length;if(after.length)targetAnnotated++;targetContext+=after.filter(e=>'inThisPlay'in e).length;stageDirectionEntries+=after.filter(e=>e.category==='stage').length;
 const beforeKeys=new Set(before.map(key));for(const e of after){if(!beforeKeys.has(key(e)))newRows.push(e);const d=dmap.get(lookup(e.lemma));if(d&&String(d.pos||'').includes('固有名詞'))proper.add(lookup(e.lemma));if('inThisPlay'in e&&d){const tags=(d.tags||[]).map(norm);if(tags.some(t=>['polysemy','context','dated-sense'].includes(t))||/[①②③]/.test(String(d.meaning||'')))polyContext++;}}
}
