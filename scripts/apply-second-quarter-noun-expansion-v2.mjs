...
const FILES = {
  ...
  patchMeta: 'data/second-quarter-noun-expansion-v2/meta.json',
  patchParts: ['data/second-quarter-noun-expansion-v2/part-01.json','data/second-quarter-noun-expansion-v2/part-02.json','data/second-quarter-noun-expansion-v2/part-03.json','data/second-quarter-noun-expansion-v2/part-04.json'],
  report: 'data/second-quarter-noun-expansion-v2-qa.json'
};

const patch = readJson(FILES.patchMeta);
patch.lineRules = FILES.patchParts.flatMap(readJson);
patch.dictionaryOnly = [];
if (patch?.schemaVersion !== 1 || patch?.patchId !== 'second-quarter-noun-expansion-v2-2026-09-01') fail('Unexpected patch identity');
const scope = patch.scope || {};
if (JSON.stringify(scope.globalSpeechRange) !== JSON.stringify([292, 582]) || scope.speechCount !== 291) fail('Unexpected patch scope');
if (scope.first !== 'act1-scene2-speech-0102' || scope.last !== 'act2-speech-0056' || scope.nextExcluded !== 'act2-speech-0057') fail('Unexpected scope boundary');
if (!Array.isArray(patch.lineRules) || patch.lineRules.length < 150) fail('Part 2 expansion rule set unexpectedly small');
...
const targetSpeeches = [...script['act1-scene2'].speeches.slice(101), ...script['act2'].speeches.slice(0, 56)];
...
for (const rule of patch.lineRules) {
  ...
  if (allowed && [...allowed].some(id => !targetIds.has(id))) fail(`Rule speechIds escape Part 2: ${surface}`);
...
if (stats.newVocabularyItems < 1) fail('Part 2 expansion produced no new line vocabulary');
