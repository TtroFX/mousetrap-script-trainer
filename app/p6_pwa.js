...
function applyWaiting(reg){if(!reg.waiting)return false;showStatus('更新版を適用しています…');reg.waiting.postMessage({type:'SKIP_WAITING'});return true}
...
navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!hadController){hadController=true;return}if(reloading)return;reloading=true;location.reload()});
navigator.serviceWorker.register('./sw.js',{scope:'./',updateViaCache:'none'}).then(reg=>{state.registration=reg;expose({supported:true});applyWaiting(reg);reg.addEventListener('updatefound',()=>{const worker=reg.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='installed'&&navigator.serviceWorker.controller)applyWaiting(reg)})});setTimeout(()=>reg.update().catch(()=>{}),3000)}).catch(err=>{console.error('PWA service worker registration failed',err);showStatus('Offline support could not start. Online learning remains available.','Retry',()=>location.reload());expose({supported:true,error:String(err?.message||err)})});
})();
