'use strict';
const BUILD_ID='index-zero-2026-08-27-r8';
...
async function networkFirst(request,cacheName,fallback=''){
  const key=relative(request)||request;
  try{
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    try{
      ...
      await Promise.allSettled(SHELL.map(async asset=>{const response=await timeoutFetch(new Request(asset,{cache:'reload'}),6000);if(response.ok)await cache.put(asset,response)}));
    }catch{}
  })());
});
self.addEventListener('activate',event=>{
  ...
  Promise.resolve().then(async()=>{
    try{const names=await caches.keys();await Promise.allSettled(names.filter(name=>name.startsWith('mts-zero-')&&!name.endsWith(BUILD_ID)).map(name=>caches.delete(name)));}catch{}
  });
});
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){event.respondWith(networkFirst(event.request,SHELL_CACHE,'./offline.html'));return;}
