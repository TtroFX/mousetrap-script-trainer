...
const SHELL_ASSETS=[
  './index.html','./p5.css','./bootstrap_entry.js','./p5_app.js','./practice_navigation.js','./reader_sheet.js','./p6_private_data.js','./pages_private_import.js','./p6_pwa.css','./p6_pwa.js',
...
self.addEventListener('message',event=>{const m=event.data||{};if(m.type==='SKIP_WAITING'){self.skipWaiting();return}if(m.type==='GET_VERSION'){const payload={type:'MTS_PWA_VERSION',buildId:BUILD_ID,dataVersion:DATA_VERSION};if(event.ports?.[0])event.ports[0].postMessage(payload);else if(event.source)event.source.postMessage(payload)}});
