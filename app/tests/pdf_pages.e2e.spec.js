...
test('Script canonical render shows p.2 in Full, Mine, and Cue Focus',async({page})=>{
  await ready(page);
  await page.goto(BASE+'#/script');
  let row=page.locator(`[data-line="${LINE}"]`);
  await expect(row).toBeVisible();
  await expectPageBadge(row,2);
...
test('Line Detail renders p.2 and compact accessible reading controls',async({page})=>{
  await ready(page);
...
test('Line Detail prev, next, close, and Script return remain intact',async({page})=>{
  ...
  await expectPageBadge(page.locator(`[data-line="${LINE}"]`),2);
});
