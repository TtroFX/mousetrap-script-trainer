...
const metrics={cacheHits:0,networkFetches:0,hashChecks:0};
...
async function openVersionCache(contract){if(!('caches'in globalThis))return null;try{return await caches.open(contract.cacheName)}catch{return null}}
async function cachedResponse(cache,name){if(!cache)return null;for(const key of [`./${name}`,new URL(name,location.href).href]){const response=await cache.match(key);if(response?.ok)return response}return null}
async function getVerifiedResponse(path){
  ...
  responseMemo.set(name,response.clone());return response.clone();
}
...
window.MTS_PRIVATE_DATA=Object.freeze({version:4,files:FILES,loadContract,getVerifiedResponse,getVerifiedJson,prepare,validateAll,getStatus,getMetrics});
