...
export class StateStore extends EventTarget {
  ...
  markReaderSeen(sceneId, lineId) { const speech = this.data.getSpeech(sceneId, lineId); if (!speech) return false; const p = this.readerProgress(), role = this.role(); if (!p.globalSeen.includes(lineId)) p.globalSeen.push(lineId); if (role && speech.speaker === role) { if (!Array.isArray(p.roles[role])) p.roles[role] = []; if (!p.roles[role].includes(lineId)) p.roles[role].push(lineId); } const updatedAt = stamp(); p.last = { sceneId, lineId, role, updatedAt }; p.updatedAt = updatedAt; jsonSet(STORAGE_KEYS.readerProgress, p); jsonSet(STORAGE_KEYS.readerLast, p.last); jsonSet(STORAGE_KEYS.lineCurrent, { sceneId, lineId }); this.emit('change', { key: 'readerProgress', value: p }); return true; }
  ...
  saveCueState(sceneId, role, state) { const all = this.cueStates(); all[this.sessionKey(sceneId, role)] = { sceneId, character: role, ...state, updatedAt: stamp() }; jsonSet(STORAGE_KEYS.cueState, all); const current = all[this.sessionKey(sceneId, role)]; jsonSet(STORAGE_KEYS.practiceLast, { mode: 'cue', sceneId, current: Number(current.index || 0) + 1, total: Number(current.total || 0), character: role, lineId: current.speechId || '', updatedAt: current.updatedAt }); return current; }
  ...
  saveRehearsalState(sceneId, role, state) { const all = this.rehearsalStates(), updatedAt = stamp(); all[this.sessionKey(sceneId, role)] = { sceneId, character: role, ...state, updatedAt }; jsonSet(STORAGE_KEYS.rehearsalState, all); jsonSet(STORAGE_KEYS.practiceLast, { mode: 'rehearsal', sceneId, current: Math.min(Number(state.index || 0) + 1, Number(state.total || 0)), total: Number(state.total || 0), character: role, lineId: state.speechId || '', completed: !!state.finished, updatedAt }); return all[this.sessionKey(sceneId, role)]; }
  ...
  sanitize() { const scene = this.selectedScene(); safeSet(STORAGE_KEYS.selectedScene, scene); const rawRole = safeGet(STORAGE_KEYS.character); if (rawRole && !CAST.includes(rawRole)) safeRemove(STORAGE_KEYS.character); this.setReaderMode(this.readerMode()); if (!this.data.hasCore()) return; const validLine = (sceneId, lineId) => !!this.data.getSpeech(sceneId, lineId); const current = safeParse(safeGet(STORAGE_KEYS.lineCurrent), null); if (current && (!sceneIds.has(current.sceneId) || !validLine(current.sceneId, current.lineId))) safeRemove(STORAGE_KEYS.lineCurrent); const pending = this.getPracticePending(); if (pending && (!sceneIds.has(pending.sceneId) || (pending.lineId && !validLine(pending.sceneId, pending.lineId)))) this.clearPracticePending(); const p = this.readerProgress(), validIds = this.data.speechById; p.globalSeen = p.globalSeen.filter(id => validIds.has(id)); for (const key of Object.keys(p.roles)) { if (!CAST.includes(key) || !Array.isArray(p.roles[key])) delete p.roles[key]; else p.roles[key] = p.roles[key].filter(id => validIds.has(id)); } if (p.last && (!sceneIds.has(p.last.sceneId) || !validLine(p.last.sceneId, p.last.lineId))) p.last = null; jsonSet(STORAGE_KEYS.readerProgress, p); }
}
