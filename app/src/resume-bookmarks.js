export class ResumeBookmarksUI {
  ...
  decorateHome() {
    ...
    if (latest) {
      ...
      section.innerHTML = `<div class="eyebrow">Continue</div><div class="resume-main"><div><h2>${this.esc(this.resumeTitle(latest))} の続き</h2><p>${this.esc(this.resumeMeta(latest))}</p></div><button class="primary-btn" type="button" data-resume-primary>続きから</button></div>${practice && practice.kind !== latest.kind ? `<button class="resume-practice-link" type="button" data-resume-practice>Practiceを再開 · ${this.esc(this.resumeTitle(practice))}</button>` : ''}`;
    ...
    bookmarkCard.innerHTML = `<div><div class="eyebrow">Bookmarks</div><h3>${count} ${count === 1 ? 'line' : 'lines'} saved</h3><p class="muted">覚えにくい台詞や後で見返したい台詞をまとめます。</p></div><button class="ghost-btn" type="button" data-open-bookmarks>Bookmark一覧</button>`;
  ...
  bookmarkToggle(sceneId, lineId, className = '') {
    ...
    el.setAttribute('aria-label', active ? 'Bookmarkを削除' : 'Bookmarkに追加');
    ...
    const toggle = event => {
      ...
      el.textContent = now ? '★' : '☆';
      el.setAttribute('aria-label', now ? 'Bookmarkを削除' : 'Bookmarkに追加');
      this.showToast(now ? 'Bookmarkに追加しました' : 'Bookmarkを削除しました', result?.removed || null);
    };
  ...
  renderBookmarks(q = new URLSearchParams()) {
    ...
    if (!this.store.hasCore()) {
      this.chrome('<section class="card"><div class="eyebrow">Bookmarks</div><h2>Bookmark一覧</h2><p class="muted">台本データを読み込んでいます…</p></section>', 'more');
      return;
    }
    ...
    this.app.querySelectorAll('[data-bookmark-remove]').forEach(button => button.onclick = () => {
      ...
      this.showToast('Bookmarkを削除しました', removed || null);
    });
